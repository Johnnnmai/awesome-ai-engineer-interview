...
Build a reusable evaluation harness for LLM-powered applications: golden datasets, automated scoring, regression detection, and a dashboard to track quality over time.

Skills demonstrated:

- Evaluation dataset design and curation
- LLM-as-judge implementation (prompt design, calibration, confidence)
- Automated regression detection (score deltas across model versions)
- Metrics: BLEU/ROUGE baselines, semantic similarity, task-specific rubrics
- CI integration to block regressions before deployment
- Observability dashboard (latency, cost, quality per prompt version)

Complexity: intermediate. Evals are the #1 differentiator in AI engineering interviews — most candidates skip them entirely. A working eval harness with before/after comparisons is a strong signal.


## AI Agent Builder Toolkit

Build a minimal, opinionated framework for constructing single-purpose AI agents: define tools, run the agent loop, handle retries and timeouts, and emit structured traces.

Skills demonstrated:

- Agent loop implementation from scratch (no LangChain)
- Tool schema design and validation (JSON Schema, Pydantic)
- Error recovery: retry budgets, fallback tools, graceful degradation
- Trace logging: per-step input/output, latency, token usage
- Prompt version management and A/B comparison
- Unit tests for deterministic tool paths; LLM-as-judge for open-ended outputs

Complexity: intermediate to advanced. Building from scratch (rather than wrapping a framework) demonstrates deep understanding of how agents work — a common interview differentiator.
